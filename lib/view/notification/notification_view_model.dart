...
class NotificationViewModel extends BaseViewModel<NotificationViewModel> {
  ...
  Widget getUsersAlerts() {
    return StreamBuilder(stream: stream, builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot) {
      if (snapshot.hasError) {
        ...
        return ListView.builder(itemCount: list.length,itemBuilder: (BuildContext context, index) {
          return AlertListItem(ktCardItem: list[index], onDelete: () => deleteAlert());
        });
      }
      else {
        return const Center(
  ...
  void deleteAlert() {
    try{
      users.doc(getCurrentUser()).update({
        "alertedId": FieldValue.arrayRemove([_resultId.value])
      });
      getUsersAlerts();
    }catch (e) {
      showToastMessage(e.toString());
